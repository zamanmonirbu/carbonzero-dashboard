...
const companies: Company[] = Array.from({ length: 50 }, (_, i) => ({
  id: uuidv4(),
  companyId: `${["KK6ZH8", "X9G5N2", "N8CZ7D", "F5KZ4E", "ZFJ8Y7", "8XK9DY", "KJ7H5B", "E6JX9G", "D4X6BC", "4YEXBK"][i % 10]}`,
  name: `${["Acme Co.", "Barone LLC.", "Binford Ltd.", "Big Kahuna Burger Ltd.", "Abstergo Ltd.", "Biffco Enterprises Ltd."][i % 6]}`,
  phone: `(${Math.floor(Math.random() * 900) + 100}) 555-${String(Math.floor(Math.random() * 10000)).padStart(4, "0")}`,
  email: `${["ziar", "joie", "codence", "xterris", "dric", "warn", "redaniel", "hamli", "inabela", "bertou", "osgoodwy", "ahana", "icadahli", "iline", "cido", "dowdy", "kspiter", "mobilelp", "webdragon", "grossman"][i % 20]}@${["gmail.com", "yahoo.com", "mail.ru", "yandex.ru", "live.com", "msn.com", "mac.com"][i % 7]}`,
...
const subscriptions: Subscription[] = Array.from({ length: 50 }, (_, i) => ({
  ...
  feature:
    i % 5 === 0 ? "Market Growth Solution\nGreat Customer Support" : "Market Growth Solution\nTime Series Models",
  amount: "$400.00",
...
const notifications: Notification[] = Array.from({ length: 40 }, (_, i) => ({
  ...
  description: [
    `You have received a new payment of $${Math.floor(Math.random() * 500) + 100}.00 from Sarah Wilson`,
...
const pendingPayments: PendingPayment[] = Array.from({ length: 30 }, (_, i) => ({
  id: uuidv4(),
  paymentId: `#PAY-${2025000 + i}`,
  user: {
    name: "Sarah Wilson",
...
export const apiService = {
  ...
  // Companies
  getCompanies: async (page = 1, limit = 10, search = ""): Promise<{ data: Company[]; total: number }> => {
    try {
      // Simulate API delay
      await new Promise((resolve) => setTimeout(resolve, 500))
  ...
  updateCompany: async (id: string, company: Partial<Company>): Promise<Company | null> => {
    try {
      // Simulate API delay
      await new Promise((resolve) => setTimeout(resolve, 800))
  ...
  // Subscriptions
  getSubscriptions: async (page = 1, limit = 10, search = ""): Promise<{ data: Subscription[]; total: number }> => {
    try {
      // Simulate API delay
      await new Promise((resolve) => setTimeout(resolve, 500))
  ...
  updateSubscription: async (id: string, subscription: Partial<Subscription>): Promise<Subscription | null> => {
    try {
      // Simulate API delay
      await new Promise((resolve) => setTimeout(resolve, 800))
  ...
  getPayments: async (
    ...
    try {
      ...
      if (filters.fromDate) {
        ...
        filteredPayments = filteredPayments.filter((payment) => payment.date >= filters.fromDate!)
      }

      if (filters.toDate) {
        // Simple date filtering for demo
        filteredPayments = filteredPayments.filter((payment) => payment.date <= filters.toDate!)
      }
  ...
  // Blogs
  getBlogs: async (page = 1, limit = 10, search = ""): Promise<{ data: Blog[]; total: number }> => {
    try {
      // Simulate API delay
      await new Promise((resolve) => setTimeout(resolve, 500))
  ...
  // Notifications
  getNotifications: async (page = 1, limit = 10): Promise<{ data: Notification[]; total: number }> => {
    try {
      // Simulate API delay
      await new Promise((resolve) => setTimeout(resolve, 500))
  ...
  getPendingPayments: async (
    ...
    try {
      ...
      if (filters.amountRange && filters.amountRange !== "All Amounts") {
        ...
        const [min, max] = filters.amountRange.split(" - ").map((a) => Number.parseInt(a.replace(/[^0-9]/g, "")))
  ...
  login: async (
    email: string,
    password: string,
  ): Promise<{ success: boolean; user?: { name: string; email: string; role: string } }> => {
    try {
      // Simulate API delay
